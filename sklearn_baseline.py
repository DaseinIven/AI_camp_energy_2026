...
output_price_path = os.path.join(output_dir, f'sklearn_baseline_output_{timestamp}.csv')
output_power_path = os.path.join(output_dir, f'output_{timestamp}.csv')  # 最终提交文件

# 边界条件特征列（与测试集对齐，仅使用预测值列）
feature_cols = ['风电预测值', '光伏预测值', '非市场化机组预测值',
                '水电预测值', '系统负荷预测值', '净负荷', '新能源渗透率']
target_col = 'A'
...
if __name__ == '__main__':
    ...
    df_train['净负荷'] = df_train['系统负荷预测值'] - (df_train['风电预测值'] + df_train['光伏预测值'])
    df_train['新能源渗透率'] = (df_train['风电预测值'] + df_train['光伏预测值']) / df_train['系统负荷预测值'].replace(0, np.nan)
    df_train['新能源渗透率'] = df_train['新能源渗透率'].fillna(0)

    # 时间特征独热编码
    time_categories = {'hour': list(range(24)), 'dayofweek': list(range(7)), 'month': list(range(1, 13))}
    for col, cats in time_categories.items():
        df_train[col] = pd.Categorical(df_train[col], categories=cats)
    df_train = pd.get_dummies(df_train, columns=list(time_categories.keys()), drop_first=True)
    ohe_cols = [c for c in df_train.columns if c.startswith(('hour_', 'dayofweek_', 'month_'))]
    all_features = feature_cols + ohe_cols
    ...
    df_test['净负荷'] = df_test['系统负荷预测值'] - (df_test['风电预测值'] + df_test['光伏预测值'])
    df_test['新能源渗透率'] = (df_test['风电预测值'] + df_test['光伏预测值']) / df_test['系统负荷预测值'].replace(0, np.nan)
    df_test['新能源渗透率'] = df_test['新能源渗透率'].fillna(0)

    # 时间特征独热编码（与训练集保持一致）
    for col, cats in time_categories.items():
        df_test[col] = pd.Categorical(df_test[col], categories=cats)
    df_test = pd.get_dummies(df_test, columns=list(time_categories.keys()), drop_first=True)
